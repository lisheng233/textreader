...
public class FilePickerActivity extends AppCompatActivity {
    ...
    private void updateList() {
        ...
        for (File file : fileList) {
            if (file.isDirectory()) {
                ...
                String encryptMark = file.getName().endsWith(".enc") ? " 🔒" : "";
                names.add("📄 " + file.getName() + encryptMark + " (" + fileSize + ")");
            }
        }
    ...
    private void showFileOptions(File file) {
        new AlertDialog.Builder(this)
            ...
            .setPositiveButton("查看", (dialog, which) -> {
                Intent intent = new Intent(FilePickerActivity.this, FileViewerActivity.class);
                intent.putExtra("file_path", file.getAbsolutePath());
                intent.putExtra("file_name", file.getName());
                startActivity(intent);
            })
            .setNeutralButton("加密", (dialog, which) -> {
                File encryptedFile = new File(file.getParent(), file.getName() + ".enc");
                EncryptDialog dialogFragment = new EncryptDialog(file, encryptedFile, 
                    (success, message) -> {
                        Toast.makeText(FilePickerActivity.this, message, Toast.LENGTH_LONG).show();
                        if (success) {
                            // 刷新文件列表
                            fillFileList(currentDir);
                        }
    ...
    private void showDecryptOption(File file) {
        String originalName = file.getName().replace(".enc", "");
        File decryptedFile = new File(file.getParent(), originalName);
        
        DecryptDialog dialogFragment = new DecryptDialog(file, decryptedFile, 
            (success, message) -> {
                Toast.makeText(FilePickerActivity.this, message, Toast.LENGTH_LONG).show();
                if (success) {
                    // 刷新文件列表
        ...
        dialogFragment.show(getSupportFragmentManager(), "decrypt_dialog");
    }
